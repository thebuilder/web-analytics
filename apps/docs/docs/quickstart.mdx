...
If the test fails, see [troubleshooting](/reference/troubleshooting). A panel missing from the dashboard later is a separate thing. It means the provider does not report that dimension, not that the connection broke.
...
- [Understanding your reports](/guides/reports) explains what each metric, breakdown, and control on the dashboard means.
- [Document analytics](/guides/document-analytics) puts a report in the workspace of a mapped document, so editors see it while editing.
